...
After editing source files, check for API documentation files in these locations (relative to the target path or repo root):
...
3. `docs/openapi.yaml`, `docs/swagger.yaml`
4. `api/openapi.yaml`, `spec/openapi.yaml`

If found, and if any **endpoint** was documented in this run:
- For each endpoint: add or update the path/operation entry under `paths`.
- Preserve existing fields — only add or update, never delete.
- Use the OpenAPI operation object prepared in Step 3.
- For YAML files: preserve the existing structure and formatting.

Minimal operation object format:
```yaml
/users:
  get:
...
Search for `*.postman_collection.json` in the root and in `postman/`, `collections/` subdirectories.

If found, and if any endpoint was documented:
- For each endpoint: find the matching request by method + URL, or add a new one.
- Set the request name and description from the generated docstring.
...
API docs updated:
  - docs/openapi.yaml      → GET /users, POST /users, DELETE /users/{id}, GET /items, POST /items
  - api.postman_collection.json → 5 requests updated
